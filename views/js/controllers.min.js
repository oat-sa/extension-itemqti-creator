...
_setupOrientationSelectors(),_setupViewSelector(),_setupThemeSelector(),_setupClosers(),_setupScreenSize(),_computeScaleFactor(),_setPreviewType(previewType),_setOrientation(orientation),$window.on("resize orientationchange",function(){_setupScreenSize(),_updateStandardPreviewSize(),_computeScaleFactor(),_scale()}),itemUri=_itemUri,overlay};return{init:init,show:show}}),define("itemqtiCreator/qtiPreview/previewLauncher",["lodash","module","taoItems/preview/preview"],function(_,module,preview){"use strict";var previewLauncher=function(previewUrl){var config=module.config();if(previewUrl=previewUrl||config.previewUrl,_.isEmpty(previewUrl))throw new TypeError("Unable to to run the preview without a launch URL");
preview.init(config.previewUrl),preview.show()};return previewLauncher}),define("itemqtiCreator/controller/QtiPreview/index",["ui/feedback","itemqtiCreator/qtiPreview/previewLauncher"],function(feedback,previewLauncher){"use strict";var qtiPreviewIndexController={start:function(){try{previewLauncher()}catch(err){feedback().error("It seems the preview of the item couldn't be launched : "+err.message)}}};return qtiPreviewIndexController});
